...
class Expense_category extends CI_Controller {
      ...
      */
  	public function index() {   
		$data["view_data"]= $this->Expense_category_model->get_data();
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			unset($data['save']);
			unset($data['id']);
			$this->Expense_category_model->updateRow('expense_category', 'expense_category_id', $this->input->post('id'), $data);
			...
			$this->Expense_category_model->insertRow('expense_category', $data);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Expense_category_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Expense_category_model->delete_data($value);		
		}
		redirect(base_url().'expense_category', 'refresh');
  	}
      ...
      */
  	public function delete_data($id) { 
		$this->Expense_category_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		...
		$columns 	= array(
array( 'db' => 'expense_category_id', 'dt' => 0 ),
array( 'db' => 'category_name', 'dt' => 1 ),
array( 'db' => 'expense_category_id', 'dt' => 2 )
);
		$joinQuery 	= '';
      ...
      */
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where = " DATE_FORMAT(`expense_category`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`expense_category`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]= $this->expense_category_model->get_data($where);
